...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # motion_noise   motion confidence
    # measurement_noise   measurement confidence
    omega_x = [[0.0 for row in range(total)] for col in range(total)]
    xi_x = [[0.0 for row in range(1)] for col in range(total)]
    omega_y = [[0.0 for row in range(total)] for col in range(total)]
    xi_y = [[0.0 for row in range(1)] for col in range(total)]
    omega_x[0][0] += 1.
    xi_x[0][0] += 50.
    omega_y[0][0] += 1.
    xi_y[0][0] += 50.
    for locI in range(len(data)):
        for measI in range(len(data[locI][0])):
            ...
            omega_x[locI][locI] += measurement_noise
            omega_x[lmI][lmI] += measurement_noise
            omega_x[locI][lmI] += -measurement_noise
            omega_x[lmI][locI] += -measurement_noise
            omega_y[locI][locI] += measurement_noise
            omega_y[lmI][lmI] += measurement_noise
            omega_y[locI][lmI] += -measurement_noise
            omega_y[lmI][locI] += -measurement_noise
            xi_x[locI][0] += -msx
            xi_x[lmI][0] += msx
            xi_y[locI][0] += -msy
            xi_y[lmI][0] += msy
        ...
        omega_x[locI][locI] += motion_noise
        omega_x[locI+1][locI+1] += motion_noise
        omega_x[locI+1][locI] += -motion_noise
        omega_x[locI][locI+1] += -motion_noise
        omega_y[locI][locI] += motion_noise
        omega_y[locI+1][locI+1] += motion_noise
        omega_y[locI][locI+1] += -motion_noise
        omega_y[locI+1][locI] += -motion_noise
        xi_x[locI][0] += -dx
        xi_x[locI+1][0] += dx
        xi_y[locI][0] += -dy
        xi_y[locI+1][0] += dy

    mu_x = matrix(omega_x).inverse() * matrix(xi_x)
    mu_y = matrix(omega_y).inverse() * matrix(xi_y)
    mu = []
    for i in range(mu_x.dimx):
        mu.append(mu_x.value[i])
        mu.append(mu_y.value[i])
    mu = matrix(mu)
